...
wrangleData_GyrProd_agg <- function(minYear, maxYear) {
  ...
  gyr_data <- gyr_data_raw %>%
    ...
    dplyr::mutate(
      ...
      monitored = ifelse(is.na(chicks), 0, 1)  # monitored if chicks was recorded
    )
  ...
  # Initialize arrays
  chicksTot     <- array(NA, dim = c(n_areas, n_years))
  ...
  for (a in seq_along(areas)) {
    ...
    for (t in 1:n_years) {
      ...
      chicksTot[a, t]   <- sum(year_df$chicks, na.rm = TRUE)
      n_monitored[a, t] <- sum(year_df$monitored, na.rm = TRUE)
    }
  }
  
  # Drop dimension names
  dimnames(chicksTot)   <- NULL
