...
#' @examples
#' 
summarisePost_areas_gyr_integ <- function(mcmc.out, 
  ...
  rRep.sum <- pSurv.sum <- betaR.sum <- detect.sum <- occ.sum <- prod.sum <- betaGP.sum <- betaOcc.sum <- betaProd.sum <- data.frame()
  ...
  for(i in 1:N_areas){
    ...
    # # Summarize average totDens
    # totDens_name <- paste0("totDens_raw[",  i, "]")
    # totDens_add <- data.frame(Area = area_names[i],
    #                         Median = median(out.mat[, totDens_name]),
    #                         lCI = unname(quantile(out.mat[, totDens_name], probs = 0.025)),
    #                         uCI = unname(quantile(out.mat[, totDens_name], probs = 0.975)),
    #                         Mean = mean(out.mat[, totDens_name]),
    #                         SD = sd(out.mat[, totDens_name]),
    #                         CV = sd(out.mat[, totDens_name])/mean(out.mat[, totDens_name])
    # )
    # totDens.sum <- rbind(totDens.sum, totDens_add)
    # 
    # # Summarize average GyrPressure
    # gyr_name <- paste0("GyrPressure_raw[",  i, "]")
    # gyr_add <- data.frame(Area = area_names[i],
    #                         Median = median(out.mat[, gyr_name]),
    #                         lCI = unname(quantile(out.mat[, gyr_name], probs = 0.025)),
    #                         uCI = unname(quantile(out.mat[, gyr_name], probs = 0.975)),
    #                         Mean = mean(out.mat[, gyr_name]),
    #                         SD = sd(out.mat[, gyr_name]),
    #                         CV = sd(out.mat[, gyr_name])/mean(out.mat[, gyr_name])
    # )
    # gyr.sum <- rbind(gyr.sum, gyr_add)
    
    # Summarise average gyrfalcon occupancy probability
    occ_name <- paste0("alphaPtar.Occ[",  i, "]")
    occ_add <- data.frame(Area = area_names[i],
                          Median = median(out.mat[, occ_name]),
                          lCI = unname(quantile(out.mat[, occ_name], probs = 0.025)),
                          uCI = unname(quantile(out.mat[, occ_name], probs = 0.975)),
                          Mean = mean(out.mat[, occ_name]),
                          SD = sd(out.mat[, occ_name]),
                          CV = sd(out.mat[, occ_name])/mean(out.mat[, occ_name])
    )
    occ.sum <- rbind(occ.sum, occ_add)
    
    # Summarise average gyrfalcon productivity per occupied territory
    prod_name <- paste0("alphaPtar.Prod[",  i, "]")
    prod_add <- data.frame(Area = area_names[i],
                          Median = median(out.mat[, prod_name]),
                          lCI = unname(quantile(out.mat[, prod_name], probs = 0.025)),
                          uCI = unname(quantile(out.mat[, prod_name], probs = 0.975)),
                          Mean = mean(out.mat[, prod_name]),
                          SD = sd(out.mat[, prod_name]),
                          CV = sd(out.mat[, prod_name])/mean(out.mat[, prod_name])
    )
    prod.sum <- rbind(prod.sum, prod_add)
    
    betaGP_name <-"betaGyr.S"
    betaGP_add <- data.frame(Area = area_names[i],
                          Median = median(out.mat[, betaGP_name]),
                          lCI = unname(quantile(out.mat[, betaGP_name], probs = 0.025)),
                          uCI = unname(quantile(out.mat[, betaGP_name], probs = 0.975)),
                          Mean = mean(out.mat[, betaGP_name]),
                          SD = sd(out.mat[, betaGP_name]),
                          CV = sd(out.mat[, betaGP_name])/mean(out.mat[, betaGP_name])
    )
    betaGP.sum <- rbind(betaGP.sum, betaGP_add)
    
    betaOcc_name <-"betaPtar.Occ"
    betaOcc_add <- data.frame(Area = area_names[i],
                             Median = median(out.mat[, betaOcc_name]),
                             lCI = unname(quantile(out.mat[, betaOcc_name], probs = 0.025)),
                             uCI = unname(quantile(out.mat[, betaOcc_name], probs = 0.975)),
                             Mean = mean(out.mat[, betaOcc_name]),
                             SD = sd(out.mat[, betaOcc_name]),
                             CV = sd(out.mat[, betaOcc_name])/mean(out.mat[, betaOcc_name])
    )
    betaOcc.sum <- rbind(betaOcc.sum, betaOcc_add)
    
    betaProd_name <-"betaPtar.Prod"
    betaProd_add <- data.frame(Area = area_names[i],
                             Median = median(out.mat[, betaProd_name]),
                             lCI = unname(quantile(out.mat[, betaProd_name], probs = 0.025)),
                             uCI = unname(quantile(out.mat[, betaProd_name], probs = 0.975)),
                             Mean = mean(out.mat[, betaProd_name]),
                             SD = sd(out.mat[, betaProd_name]),
                             CV = sd(out.mat[, betaProd_name])/mean(out.mat[, betaProd_name])
    )
    betaProd.sum <- rbind(betaProd.sum, betaProd_add)
  }
  
  
  
  ## Summarize hyper-parameters
  hParams.list <- c(#"h.Mu.R", "h.sigma.R", "sigmaT.R", 
                    "sigmaR.R",
                    #"h.Mu.betaR.R", "h.sigma.betaR.R",
                    #"h.Mu.S","h.sigma.S", "sigmaT.S", 
                    "sigmaR.S",
                    #"h.mu.dd", "h.sigma.dd", 
                    #"sigmaT.dd", 
                    "sigmaR.dd",
  ...
  PostSum.list <- list(rRep.sum = rRep.sum,
                       ...
                       # totDens.sum = totDens.sum,
                       # gyr.sum = gyr.sum,
                       occ.sum = occ.sum,
                       prod.sum = prod.sum,
                       betaGP.sum = betaGP.sum,
                       betaOcc.sum = betaOcc.sum,
                       betaProd.sum = betaProd.sum,
