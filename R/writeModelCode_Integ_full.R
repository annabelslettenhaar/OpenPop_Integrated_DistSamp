...
writeModelCode_Integ_full <- function(survVarT, telemetryData){
  ...
  IDSM.code <- nimble::nimbleCode({
    ...
    for(x in 1:N_areas){
      ...
      if(survVarT){
        #logit(S[x, 1:(N_years-1)]) <- logit(Mu.S[x] + epsR.S[x, 1:(N_years-1)]) # Old version
        logit(S[x, 1:(N_years-1)]) <- logit(Mu.S[x]) + epsR.S[x, 1:(N_years-1)] + betaGyr.S*GyrPressure_std[x, 1:(N_years-1)]
      }else{
        logit(S[x, 1:(N_years-1)]) <- logit(Mu.S[x]) + betaGyr.S*GyrPressure_std[x, 1:(N_years-1)]
      }
