...
prepareInputData_Integ_full <- function(d_trans, d_obs, d_rodent, #d_gyr,
                                   d_gyrocc, d_gyrprod,
                                   d_SD, 
  ...
  if(areaAggregation){
    colnames(d_trans)[which(colnames(d_trans) == "gyrArea")] <- "spatialUnit"
    colnames(d_obs)[which(colnames(d_obs) == "gyrArea")] <- "spatialUnit"
  }else{
  ...
  N_territory <- 14 # make this dynamic?
  ...
  GyrPressure_mean <- c(12.75, 9.5, 8.466667)
  GyrPressure_sd <- c(5.320342, 5.437735, 4.411844)
  ...
  input.data <- list(
    ...
    # Gyrfalcon covariate
    GyrPressure_mean = GyrPressure_mean,
    GyrPressure_sd = GyrPressure_sd,
    #GyrPressure = d_gyr$gyrPressure,
    #SDPreBrood = weather_data$d_SD$data,
    
    # Gyrfalcon occupancy response variable
    #GyrOcc = d_gyrocc,
    terrMonitoredOcc = d_gyrocc$n_monitored,
    terrOcc = d_gyrocc$Occ_count,
  ...
  nim.data <- list(sumR_obs = input.data$sumR_obs, 
                   sumAd_obs = input.data$sumAd_obs,
                   y = input.data$y, 
  ...
  nim.constants <- list(N_years = input.data$N_years, min_years = input.data$min_years, max_years = input.data$max_years,
                        ...
                        GyrPressure_mean = input.data$GyrPressure_mean,
                        GyrPressure_sd = input.data$GyrPressure_sd)
