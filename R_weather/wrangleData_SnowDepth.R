...
wrangleData_SnowDepth <- function(minYear, maxYear, areas) {
  library(purrr)
  library(httr)
  library(jsonlite)
  library(sf)
  ...
  # 11. Filter for May 20th
  df.may20 <- df.snow %>%
    filter(format(Date, "%m-%d") == "05-20")
  
  # 12. Calculate average snow depth per area and year
  avg_snow_may20 <- df.may20 %>%
    group_by(gyrArea, Year) %>%
    summarise(avg_snow_depth = mean(Value, na.rm = TRUE), .groups = "drop")
